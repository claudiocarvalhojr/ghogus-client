...
let addItem = async (req, res) => {
    ...
    if (req.isAuthenticated()) {
        ...
        // let sku = req.body.addCartItemSKU
        ...
        res.cookie('sessionId', sessionId.token, { maxAge: ((((1000 * 30) * 1) * 1) * 1) })
        let newCart = {
        ...
        await manager.send('post', '/cart', newCart)
        let cart = await manager.find('/cart/last/' + JSON.stringify({ values: { 'sessionId': sessionId.token, 'isEnabled': true } }))
        renderCart(req, res, cart[0])
    }
}
...
let removeItem = async (req, res) => {
    utils.log('cartController.removeItem()...')
...
let freightCalculation = async (req, res) => {
    utils.log('cartController.freightCalculation()...')
...
// let cartManager = async (res, req, next) => {
// utils.log('cartController.cartManager(' + req.body.action + ')...')
// let action = req.body.action
// if (action.localeCompare('add') == 0) {
// addItem(req, res, next)
// }
// else {
//     cartManager2(req, res)
// }
// }

let cartManager = async (req, res) => {
    utils.log('cartController.cartManager2()...')
    let sku = null
...
module.exports = { cartManager, addItem, freightCalculation, findCart }